...
- **Organizations — Resource Control Policies (RCPs)** — the resource-side companion to SCPs (data perimeter). The policy management API is generalized to a second type: `CreatePolicy`/`Describe`/`Update`/`Delete`/`List*`/`Attach`/`Detach`/`EnablePolicyType`/`DisablePolicyType` all accept `RESOURCE_CONTROL_POLICY`, with `List*` filtered by type and the AWS-managed `p-RCPFullAWSAccess` default attached to the root, OUs, and accounts (and, like real AWS, non-detachable). Customer RCPs are validated to AWS's RCP syntax rules — `Effect` must be `Deny`, `Principal` must be `*` (principals are targeted via conditions), `NotPrincipal`/`NotAction` are rejected, and the `Action` must be service-scoped — and the evaluation engine gains the `Principal` element (SCP statements are unaffected). **Opt-in enforcement** — gated behind a separate `RCP_ENFORCEMENT=1` (or the `/_ministack/config` runtime endpoint; default off) — evaluates a request against the **resource owner's** RCP chain (the resource account is taken from the request's resource ARN) with the **calling** principal's context (`aws:PrincipalOrgID`, `aws:PrincipalAccount`, `aws:PrincipalArn`, `aws:SourceOrgID`), denying with `AccessDenied`, and exempts the management account's own resources. Applies to the RCP-supported services MiniStack can enforce: `s3`, `sts`, `sqs`, `secretsmanager`, `kms`, `logs`, `dynamodb`, `ecr` (`s3`/`dynamodb` are same-account only; AWS supports further RCP services that have no MiniStack resource extractor). Models the SCP/RCP permission ceiling only — IAM and resource-based policies are not enforced.
- **Organizations — Service Control Policies (SCPs)** — adds the full SCP management API (`CreatePolicy`, `DescribePolicy`, `UpdatePolicy`, `DeletePolicy`, `ListPolicies`, `ListPoliciesForTarget`, `ListTargetsForPolicy`, `AttachPolicy`, `DetachPolicy`, `EnablePolicyType`, `DisablePolicyType`, `TagResource`, `UntagResource`, `ListTagsForResource`), the AWS-managed `p-FullAWSAccess` default attached to the root, OUs, and accounts, and a policy-evaluation engine (wildcard action/resource matching, the condition grammar including the `ForAnyValue`/`ForAllValues` set operators with AWS-accurate missing-key semantics, and the decision rule that an explicit `Deny` anywhere wins while an action must otherwise be allowed at every level of the root→OU→account chain). **Opt-in enforcement** — gated behind `SCP_ENFORCEMENT=1` (or the `/_ministack/config` runtime endpoint; default off, so existing behaviour is unchanged) — evaluates a member account's data-plane calls against the org's attached SCPs and denies with `AccessDenied`, exempting the management account and requiring `FeatureSet=ALL` with the policy type enabled. Per-resource matching is best-effort for `s3`, `dynamodb`, `sqs`, `sns`, `lambda`, `secretsmanager`, `kinesis`, `firehose`, `ecr`, `ssm`, `kms`, `logs`, and `states`, falling back to wildcard elsewhere. Models the SCP permission ceiling only — IAM is not enforced, so the SCP ∩ IAM intersection is not modelled.
